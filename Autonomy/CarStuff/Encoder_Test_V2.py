...
try:
    while True:
        ...
        if left_sensor_state and not left_last_state:
            print('left connected')

        if not left_sensor_state and left_last_state:
            print('left broken')

        left_last_state = left_sensor_state

        right_sensor_state = GPIO.input(rightSensorPin)
        if right_sensor_state and not right_last_state:
            print('right connected')

        if not right_sensor_state and right_last_state:
            print('right broken')
